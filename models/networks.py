...
class BASE_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5,
        ...
        # self.Coarse_changemap=nn.Sequential(nn.Conv2d(32, 32, kernel_size=3, padding=1,bias=False),
        #                                     nn.BatchNorm2d(32),
        #                                     nn.ReLU(),
    ...
    def forward(self, x1, x2):
        ...
        x1 = self.forward_single(x1)    # (b,c,h,w)
        x2 = self.forward_single(x2)

        # # 输出应是该点变化与否的概率
        # x3=torch.abs(x1 - x2)   # 用于生成粗变化检测图
        #
        # x4=self.Coarse_changemap(x3)    # (1,2,64,64)
        # # x5=F.upsample_bilinear(x4,x3.size()[2:])    # (1,2,64,64)
        # change_feature=x4[:,1,:,:]  # 第0维是不变的，第一维是变化的
        # _, indices = change_feature.topk(k=self.k, dim=2, largest=True) # 找
...
##################################
        ...
        if self.output_sigmoid:
            x = self.sigmoid(x) # (1,2,256,256),有正负
        return x
